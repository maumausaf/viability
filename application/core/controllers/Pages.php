...
class Pages extends CI_Controller {
    ...
    public function answer($id = null) {
        ...
        $grauUser = (string) 3; //definindo grau de usuario para este metodo
        $user = $this->session->userdata('user');
        $superUser = (string) 1;
        
        if($user->user == $grauUser || $user->user == $superUser){
        $this->load->library('form_validation'); //importando biblioteca -- form_validation
    ...
    public function index() {
        ...
        $config = array(
			...
			"num_links" => 3,
			"uri_segment" => 3,
			"total_rows" => $this->pages_model->CountAll($status = 3),
        ...
        public function EnviarEmail()
    {
        ...
        $this->email->from('remetente@email.com', 'Vendas'); // Remetente
